...
CREATE TABLE IF NOT EXISTS `propane-girder-470214-h9.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `propane-girder-470214-h9.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `propane-girder-470214-h9.silver_dataset.claims` c
